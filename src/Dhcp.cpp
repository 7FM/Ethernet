...
int DhcpClass::beginWithDHCP(const uint8_t *mac, unsigned long timeout,
...
void DhcpClass::send_DHCP_MESSAGE(uint8_t messageType,
  ...
  buffer[16] = hostName;
  buffer[17] =
      strlen(HOST_NAME) + 6; // length of hostname + last 3 bytes of mac address
...
IPAddress DhcpClass::getLocalIp() const { return IPAddress(_dhcpLocalIp); }

IPAddress DhcpClass::getSubnetMask() const {
  return IPAddress(_dhcpSubnetMask);
}

IPAddress DhcpClass::getGatewayIp() const { return IPAddress(_dhcpGatewayIp); }
